...
public interface CalculateDataMapper {
	...
	@Insert("insert into calculate_data(contractId,insurance_expence,retention_ratio,retention,"
			+ "line_num,ceiling_top,pay,total values(#{contractId},#{insurance_expence},#{retention_ratio},#{retention},"
			+ "#{line_num},#{ceiling_top},#{pay},#{total}))")
	...
	@Delete("delete from calculate_data where contractId=?")
